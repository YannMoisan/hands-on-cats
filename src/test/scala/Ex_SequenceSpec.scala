class Ex_SequenceSpec extends org.specs2.mutable.Specification {
  ...
  "[scalaz] transform a Seq[Option] in Option[Seq]" >> {
    "should return Some when the seq contains only Some" >> {
      Ex_Sequence.scalaz.sequence(Seq(Some(1), Some(2))) must_== Some(List(1, 2))
    }
    "should return None when the seq contains Some and None" >> {
      Ex_Sequence.scalaz.sequence(Seq(Some(1), None)) must_== None
    }
    "should return None when the seq contains only None" >> {
      Ex_Sequence.scalaz.sequence(Seq(None, None)) must_== None
    }
  }
  "[scalaz] transform a Seq[Either] in Either[Seq]" >> {
    "should return Some when the seq contains only Some" >> {
      Ex_Sequence.scalaz.sequenceEither(Seq(Right(1), Right(2))) must_== Right(List(1, 2))
    }
    "should return None when the seq contains Some and None" >> {
      Ex_Sequence.scalaz.sequenceEither(Seq(Right(1), Left("error"))) must_== Left("error")
    }
    "should return None when the seq contains only None" >> {
      Ex_Sequence.scalaz.sequenceEither(Seq(Left("error"), Left("error"))) must_== Left("error")
    }
  }
  "[cats] transform a Seq[Option] in Option[Seq]" >> {
    "should return Some when the seq contains only Some" >> {
      Ex_Sequence.cats.sequence(Seq(Some(1), Some(2))) must_== Some(List(1, 2))
    }
    "should return None when the seq contains Some and None" >> {
      Ex_Sequence.cats.sequence(Seq(Some(1), None)) must_== None
    }
    "should return None when the seq contains only None" >> {
      Ex_Sequence.cats.sequence(Seq(None, None)) must_== None
    }
  }
  "[cats] transform a Seq[Either] in Either[Seq]" >> {
    "should return Some when the seq contains only Some" >> {
      Ex_Sequence.cats.sequenceEither(Seq(Right(1), Right(2))) must_== Right(List(1, 2))
    }
    "should return None when the seq contains Some and None" >> {
      Ex_Sequence.cats.sequenceEither(Seq(Right(1), Left("error"))) must_== Left("error")
    }
    "should return None when the seq contains only None" >> {
      Ex_Sequence.cats.sequenceEither(Seq(Left("error"), Left("error"))) must_== Left("error")
    }
  }
}
