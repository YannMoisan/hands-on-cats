...
class Ex_FutureOptionSpec(implicit ee: ExecutionEnv) extends org.specs2.mutable.Specification {
  ...
  "[scalaz] flatMap on Future[Option[Int]]" >> {
    "should sum integers when both option are defined" >> {
      Ex_FutureOption.scalaz.flatMap(futureSome, futureSome) must be_==(Some(2)).await
    }
    "should return None when the first option is None" >> {
      Ex_FutureOption.scalaz.flatMap(futureNone, futureSome) must be_==(None).await
    }
    "should return None when the second option is None" >> {
      Ex_FutureOption.scalaz.flatMap(futureSome, futureNone) must be_==(None).await
    }
    "should return None when both option are None" >> {
      Ex_FutureOption.scalaz.flatMap(futureNone, futureNone) must be_==(None).await
    }
  }

  "[cats] flatMap on Future[Option[Int]]" >> {
    "should sum integers when both option are defined" >> {
      Ex_FutureOption.cats.flatMap(futureSome, futureSome) must be_==(Some(2)).await
    }
    "should return None when the first option is None" >> {
      Ex_FutureOption.cats.flatMap(futureNone, futureSome) must be_==(None).await
    }
    "should return None when the second option is None" >> {
      Ex_FutureOption.cats.flatMap(futureSome, futureNone) must be_==(None).await
    }
    "should return None when both option are None" >> {
      Ex_FutureOption.cats.flatMap(futureNone, futureNone) must be_==(None).await
    }
  }
